...
import LatLonEllipsoidal from "geodesy/latlon-ellipsoidal-datum.js";
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
class OsGridRef {
  ...
  toLatLon(datum = LatLonEllipsoidal.datums.WGS84) {
    ...
    const XIIA =
      (secφ / (5040 * ν7)) * (61 + 662 * tan2φ + 1320 * tan4φ + 720 * tan6φ);
    const dE = E - E0,
      dE2 = dE * dE,
      dE3 = dE2 * dE,
      dE4 = dE2 * dE2,
...
const convertOSGridPolygon = (coordinates) => {
  // Error/invalid condition - just return what we have.
  if (
    !coordinates ||
  ...
  if (isCoordinate(coordinates)) {
    ...
    const converted = gridref.toLatLon();
    const { lng, lat } = converted;
    return [lng, lat];
  }
  if (Array.isArray(coordinates)) {
    return coordinates.map((coord) => convertOSGridPolygon(coord));
  }
  return coordinates;
};
export default convertOSGridPolygon;